...
def large_mul(a,b):
    ...
    while 1:
        if i + batch > a.shape[0]:
            c[i:,:] = a[i:,:] * b[i:,:]
            break
        c[i:i+batch,:] = a[i:i+batch,:]*b[i:i+batch,:]
        i += batch
    return c
...
class scETM(BaseCellModel):
    ...
    def forward(self,
        ...
        ######### Add regularizer: #########
        # graph = torch.zeros([self.cells.shape[1], self.cells.shape[1]])
        if self.guidance_graph is None:
            loss_reg = 0
        else:
        ...
        # print(theta.shape) # (1698,50) 
        

        nll = (-recon_log * normed_cells if self.normed_loss else cells).sum(-1).mean()
        kl_delta = get_kl(mu_q_delta, logsigma_q_delta).mean()
        loss = nll + hyper_param_dict['kl_weight'] * kl_delta + reg_weight * loss_reg
        record = dict(loss=loss, nll=nll, kl_delta=kl_delta, loss_reg = loss_reg)
